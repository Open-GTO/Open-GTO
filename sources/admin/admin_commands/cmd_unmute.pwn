...
COMMAND:unmute(playerid, params[])
{
	...
	if (sscanf(params, "s[32]S()[" #MAX_MUTE_REASON_LENGTH "]", subparams, reason)) {
		SendClientMessage(playerid, -1, _(ADMIN_COMMAND_UNMUTE_HELP));
		return 1;
	}
	...
	if (strcmp(subparams, "all", true) == 0) {
		...
		SendClientMessage(playerid, -1, _(ADMIN_COMMAND_UNMUTE_TARGET_ERROR));
		return 1;
	}
	...
	if (targetid == -1) {
		format(string, sizeof(string), _(ADMIN_COMMAND_UNMUTE_ALL), playername, playerid, reason);
		if (is_with_reason) {
			format(string, sizeof(string), _(ADMIN_COMMAND_UNMUTE_REASON), string, reason);
		}
