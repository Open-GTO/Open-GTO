...
stock Player_Text_OnPlayerText(playerid, text[])
{
	...
	switch (text[0]) {
		case '!': {
			...
			Log(playerlog, INFO, "Player: %s(%d): <GANG CHAT>: %s", playername, playerid, text[start_pos]);
			return 0;
		}
		case '@', '"': {
			if (strlen(text[1]) < 2) {
			...
			Log(playerlog, INFO, "Player: %s(%d): <ADMIN TALK>: %s", playername, playerid, text[1]);
			return 0;
		}
		case '#', '¹': {
			if (strlen(text[1]) < 2) {
			...
			Log(playerlog, INFO, "Player: %s(%d): <MODERATOR TALK>: %s", playername, playerid, text[1]);
			return 0;
		}
		case '$', ';': {
			if (IsPlayerMuted(playerid)) {
			...
			Log(playerlog, INFO, "Player: %s(%d): <SAY>: %s", playername, playerid, text[1]);
			return 0;
		}
	}
	return 1;
