...
-------------------------------------------------------------------
							Open-GTO
-------------------------------------------------------------------
0.0.0 Ц 1.0.0		ZiGGi, GhostTT, heufix, Elbi
-------------------------------------------------------------------
*/
...
{	// CREDITS NOT DELETE !!!!!!!!!!!!!!!!!
	...
	print("\t\tDmitry Frolov (FP)\nGTO-Rus Team(Ѓ): GhostTT, heufix, Elbi, ZiGGi");
	print("_____________________________________________________________\n");
...
public OnPlayerText(playerid, text[])
{
	...
	switch(text[0])
	{
	    case '!':
	    {
			if(GetPVarInt(playerid,"GangID") == 0 || Player[playerid][MuteTime] != 0)    //»грок заткнут
			{
				SendClientMessage(playerid,COLOUR_RED,lang_texts[1][14]);
				return 0;
			}
	...
	if(Player[playerid][MuteTime] != 0)  //«аткнут
	{
		SendClientMessage(playerid,COLOUR_RED,lang_texts[1][14]);
		return 0;
	}
